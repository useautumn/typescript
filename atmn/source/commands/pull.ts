...
// Plans${planSnippets.join('\n')}
	...
	console.log(chalk.dim('Fetching products and features from all environments for SDK types...'));
...
async function fetchFeaturesFromAllEnvironments(): Promise<Feature[]> {
	...
	try {
		...
		if (fs.existsSync(envPath)) {
			...
			const otherKeyName = currentEnvIsProd ? 'AUTUMN_SECRET_KEY' : 'AUTUMN_PROD_SECRET_KEY';
			const otherKeyMatch = envContent.match(new RegExp(`${otherKeyName}=(.+)`));

			if (otherKeyMatch && otherKeyMatch[1]) {
				// Temporarily switch environment
				const originalArgs = [...process.argv];
				if (currentEnvIsProd) {
					process.argv = process.argv.filter(a => a !== '--prod' && a !== '-p');
				} else {
				...
				try {
					...
					console.log(chalk.dim('Could not fetch from other environment (this is okay)'));
				}
		...
		if (fs.existsSync(envPath)) {
			...
			if (otherKeyMatch && otherKeyMatch[1]) {
				...
				try {
					...
					// Silently fail if other environment is not accessible
					console.log(chalk.dim('Could not fetch products from other environment (this is okay)'));
				}
