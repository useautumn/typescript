/**
 * Features Rezi TUI App — migrated from React Ink FeaturesView
 */
import { ui, rgb, type VNode } from "@rezi-ui/core";
import { createNodeApp } from "@rezi-ui/node";
import clipboardy from "clipboardy";
import open from "open";
import { fetchFeatures } from "../../../lib/api/endpoints/features.js";
import type { ApiFeature } from "../../../lib/api/types/index.js";
import { AppEnv } from "../../../lib/env/detect.js";
import { getKey } from "../../../lib/env/keys.js";
import { APP_VERSION } from "../../../lib/version.js";
import {
	titleBar,
	bottomBar,
	loadingText,
	errorView,
	emptyView,
	keyValue,
	colors,
	formatDate,
} from "../helpers.js";

const AUTUMN_DASHBOARD_URL = "https://app.useautumn.com";
const PAGE_SIZE = 50;

type FocusTarget = "table" | "sheet" | "search";

// ── Search helper ──────────────────────────────────────────────

function featureSearchFn(f: ApiFeature, query: string): boolean {
	const q = query.toLowerCase();
	return f.id.toLowerCase().includes(q) || f.name.toLowerCase().includes(q) || f.type.toLowerCase().includes(q);
}

function paginate<T>(items: T[], page: number, pageSize: number, searchQuery: string, searchFn?: (item: T, q: string) => boolean) {
	let filtered = items;
	if (searchQuery.trim() && searchFn) {
		filtered = items.filter(i => searchFn(i, searchQuery.trim()));
	}
	const totalItems = filtered.length;
	const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
	const clampedPage = Math.min(Math.max(1, page), totalPages);
	const start = (clampedPage - 1) * pageSize;
	const pageItems = filtered.slice(start, start + pageSize);
	return { pageItems, page: clampedPage, totalPages, totalItems, hasMore: clampedPage < totalPages, hasPrev: clampedPage > 1 };
}

// ── State ──────────────────────────────────────────────────────

interface FeaturesState {
	allFeatures: ApiFeature[];
	loading: boolean;
	error: string | null;
	page: number;
	selectedIndex: number;
	selectedItem: ApiFeature | null;
	focusTarget: FocusTarget;
	sheetOpen: boolean;
	searchQuery: string;
	searchOpen: boolean;
	searchInput: string;
	copiedFeedback: boolean;
}

// ── App ────────────────────────────────────────────────────────

export interface CreateFeaturesAppOptions {
	environment?: AppEnv;
	onExit: () => void;
}

export async function createFeaturesApp(opts: CreateFeaturesAppOptions) {
	const env = opts.environment ?? AppEnv.Sandbox;

	const app = createNodeApp<FeaturesState>({ config: { executionMode: "inline" },
		initialState: {
			allFeatures: [],
			loading: true,
			error: null,
			page: 1,
			selectedIndex: 0,
			selectedItem: null,
			focusTarget: "table",
			sheetOpen: false,
			searchQuery: "",
			searchOpen: false,
			searchInput: "",
			copiedFeedback: false,
		},
	});

	// ── Data fetching ──────────────────────────────────────────

	async function loadFeatures() {
		app.update(st => ({ ...st, loading: true, error: null }));
		try {
			const secretKey = getKey(env);
			const list = await fetchFeatures({ secretKey, includeArchived: true });
			app.update(st => {
				const pg = paginate(list, st.page, PAGE_SIZE, st.searchQuery, featureSearchFn);
				const sel = pg.pageItems[st.selectedIndex] ?? pg.pageItems[0] ?? null;
				return { ...st, allFeatures: list, loading: false, selectedItem: sel, selectedIndex: Math.min(st.selectedIndex, Math.max(0, pg.pageItems.length - 1)) };
			});
		} catch (e) {
			app.update(st => ({ ...st, error: e instanceof Error ? e.message : String(e), loading: false }));
		}
	}

	// ── Computed ────────────────────────────────────────────────

	function getPageData(s: FeaturesState) {
		return paginate(s.allFeatures, s.page, PAGE_SIZE, s.searchQuery, featureSearchFn);
	}

	// ── View ───────────────────────────────────────────────────

	app.view((s) => {
		const pg = getPageData(s);
		const features = pg.pageItems;

		const pagText = pg.totalItems > 0
			? `${(pg.page - 1) * PAGE_SIZE + 1}-${Math.min(pg.page * PAGE_SIZE, pg.totalItems)} of ${pg.totalItems}`
			: "";

		const top = titleBar({ commandName: "atmn features", version: APP_VERSION, paginationText: pagText, searchQuery: s.searchQuery || undefined });

		const hints = buildHints(s, pg.hasPrev, pg.hasMore);
		const bot = bottomBar(hints);

		// Loading
		if (s.loading && s.allFeatures.length === 0) {
			return ui.column({}, [top, loadingText(`Loading ${env === AppEnv.Live ? "live" : "sandbox"} features...`), bot]);
		}
		// Error
		if (s.error && s.allFeatures.length === 0) {
			return ui.column({}, [top, errorView(s.error, { onRetry: true }), bot]);
		}
		// Empty
		if (features.length === 0 && !s.loading) {
			const title = s.searchQuery ? `No results for "${s.searchQuery}"` : "No features found";
			const desc = s.searchQuery ? "Press 'x' to clear search" : "Create features via the API or Autumn dashboard";
			return ui.column({}, [top, emptyView(title, desc, s.searchQuery || undefined), bot]);
		}

		// Search input overlay
		const searchRow = s.searchOpen
			? ui.box({ border: "rounded", px: 1 }, [
				ui.row({}, [
					ui.text("Search: ", { style: { fg: colors.magenta } }),
					ui.input({ id: "feature-search", value: s.searchInput, onInput: (v: string) => app.update(st => ({ ...st, searchInput: v })) }),
				]),
			])
			: null;

		// Table
		const tableData = features.map((f: ApiFeature) => ({
			id: f.id,
			name: f.name,
			type: f.type,
			consumable: f.consumable ? "Yes" : "No",
			status: f.archived ? "Archived" : "Active",
		}));
		const table = ui.table({
			id: "features-table",
			columns: [
				{ key: "id", header: "ID", flex: 1 },
				{ key: "name", header: "Name", flex: 1 },
				{ key: "type", header: "Type", width: 14 },
				{ key: "consumable", header: "Consumable", width: 12 },
				{ key: "status", header: "Status", width: 10 },
			],
			data: tableData,
			getRowKey: (row) => row.id,
			selection: s.selectedItem ? [s.selectedItem.id] : [],
			selectionMode: "single",
			onSelectionChange: (keys: string[]) => {
				const idx = features.findIndex(f => f.id === keys[0]);
				if (idx >= 0) app.update(st => ({ ...st, selectedIndex: idx, selectedItem: features[idx] }));
			},
			onRowPress: (_row, idx: number) => {
				const feat = features[idx];
				if (feat) app.update(st => ({ ...st, selectedIndex: idx, selectedItem: feat, sheetOpen: true, focusTarget: "sheet" }));
			},
		});

		// Side panel
		let sidePanel: VNode | null = null;
		if (s.sheetOpen && s.selectedItem) {
			sidePanel = renderFeatureSheet(s);
		}

		const body = sidePanel
			? ui.row({ flex: 1 }, [ui.column({ flex: 1 }, [table]), ui.column({ width: 44 }, [sidePanel])])
			: table;

		const parts: VNode[] = [top];
		if (searchRow) parts.push(searchRow);
		parts.push(body, bot);
		return ui.column({}, parts);
	});

	// ── Render helpers ─────────────────────────────────────────

	function renderFeatureSheet(s: FeaturesState): VNode {
		const f = s.selectedItem!;
		const borderColor = s.focusTarget === "sheet" ? colors.magenta : colors.gray;
		const children: VNode[] = [
			ui.text(f.name, { style: { bold: true } }),
			ui.text(""),
			ui.text("Basic Info", { style: { fg: colors.magenta, bold: true } }),
			keyValue("ID", f.id),
			keyValue("Name", f.name),
			keyValue("Type", f.type),
		];
		children.push(ui.row({ gap: 1 }, [
			ui.text("Consumable:", { style: { fg: colors.gray } }),
			ui.text(f.consumable ? "Yes" : "No", { style: { fg: f.consumable ? colors.green : colors.gray } }),
		]));
		children.push(ui.row({ gap: 1 }, [
			ui.text("Status:", { style: { fg: colors.gray } }),
			ui.text(f.archived ? "Archived" : "Active", { style: { fg: f.archived ? colors.red : colors.green } }),
		]));

		// Display
		if (f.display && (f.display.singular || f.display.plural)) {
			children.push(ui.text(""), ui.text("Display", { style: { fg: colors.magenta, bold: true } }));
			if (f.display.singular) children.push(keyValue("Singular", f.display.singular));
			if (f.display.plural) children.push(keyValue("Plural", f.display.plural));
		}

		// Event names (metered)
		if (f.type === "metered" && f.event_names && f.event_names.length > 0) {
			children.push(ui.text(""), ui.text("Event Names", { style: { fg: colors.magenta, bold: true } }));
			f.event_names.forEach((en, i) => {
				children.push(ui.row({}, [ui.text(`${i + 1}. `, { style: { fg: colors.gray } }), ui.text(en)]));
			});
		}

		// Credit schema
		if (f.type === "credit_system" && f.credit_schema && f.credit_schema.length > 0) {
			children.push(ui.text(""), ui.text("Credit Schema", { style: { fg: colors.magenta, bold: true } }));
			f.credit_schema.forEach((item, i) => {
				children.push(ui.row({}, [ui.text(`${i + 1}. `, { style: { fg: colors.gray } }), ui.text(item.metered_feature_id, { style: { bold: true } })]));
				children.push(ui.row({ gap: 1 }, [ui.text("   Credit Cost:", { style: { fg: colors.gray } }), ui.text(String(item.credit_cost), { style: { fg: colors.cyan } })]));
			});
		}

		// Actions
		children.push(ui.text(""));
		if (s.copiedFeedback) {
			children.push(ui.text("Copied!", { style: { fg: colors.green } }));
		} else {
			children.push(ui.row({}, [ui.text("[c]", { style: { fg: colors.magenta } }), ui.text(" Copy ID", { style: { fg: colors.gray } })]));
		}
		children.push(ui.row({}, [ui.text("[o]", { style: { fg: colors.magenta } }), ui.text(" Open in Autumn", { style: { fg: colors.gray } })]));

		return ui.box({ border: "rounded", px: 1 }, children);
	}

	function buildHints(s: FeaturesState, hasPrev: boolean, hasMore: boolean): { key: string; label: string; visible?: boolean }[] {
		if (s.focusTarget === "sheet" && s.sheetOpen) {
			return [
				{ key: "Tab", label: "focus table" },
				{ key: "Esc", label: "close" },
				{ key: "c", label: "copy ID" },
				{ key: "o", label: "open" },
				{ key: "q", label: "quit" },
			];
		}
		return [
			{ key: "↑/↓", label: "navigate" },
			{ key: "n", label: "next page", visible: hasMore },
			{ key: "p", label: "prev page", visible: hasPrev },
			{ key: "Enter", label: "inspect" },
			{ key: "/", label: "search" },
			{ key: "x", label: "clear search", visible: !!s.searchQuery },
			{ key: "r", label: "refresh" },
			{ key: "q", label: "quit" },
		];
	}

	// ── Keys ───────────────────────────────────────────────────

	app.keys({
		"q": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			app.stop();
			opts.onExit();
		},
		"escape": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) {
				app.update(st => ({ ...st, searchOpen: false, searchInput: "" }));
				return;
			}
			if (s.sheetOpen) {
				app.update(st => ({ ...st, sheetOpen: false, focusTarget: "table" }));
			}
		},
		"return": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) {
				app.update(st => ({ ...st, searchOpen: false, searchQuery: st.searchInput, page: 1, selectedIndex: 0 }));
				return;
			}
			if (s.focusTarget === "table") {
				const pg = getPageData(s);
				const feat = pg.pageItems[s.selectedIndex];
				if (feat) app.update(st => ({ ...st, sheetOpen: true, selectedItem: feat, focusTarget: "sheet" }));
			}
		},
		"tab": (ctx) => {
			const s = ctx.state;
			if (s.sheetOpen) {
				app.update(st => ({ ...st, focusTarget: st.focusTarget === "table" ? "sheet" : "table" }));
			}
		},
		"up": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				app.update(st => {
					const ni = Math.max(0, st.selectedIndex - 1);
					const pg = getPageData(st);
					return { ...st, selectedIndex: ni, selectedItem: pg.pageItems[ni] ?? null };
				});
			}
		},
		"k": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				app.update(st => {
					const ni = Math.max(0, st.selectedIndex - 1);
					const pg = getPageData(st);
					return { ...st, selectedIndex: ni, selectedItem: pg.pageItems[ni] ?? null };
				});
			}
		},
		"down": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				app.update(st => {
					const pg = getPageData(st);
					const ni = Math.min(pg.pageItems.length - 1, st.selectedIndex + 1);
					return { ...st, selectedIndex: ni, selectedItem: pg.pageItems[ni] ?? null };
				});
			}
		},
		"j": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				app.update(st => {
					const pg = getPageData(st);
					const ni = Math.min(pg.pageItems.length - 1, st.selectedIndex + 1);
					return { ...st, selectedIndex: ni, selectedItem: pg.pageItems[ni] ?? null };
				});
			}
		},
		"/": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			app.update(st => ({ ...st, searchOpen: true, searchInput: st.searchQuery, focusTarget: "search" as FocusTarget }));
		},
		"s": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen || s.focusTarget === "sheet") return;
			app.update(st => ({ ...st, searchOpen: true, searchInput: st.searchQuery, focusTarget: "search" as FocusTarget }));
		},
		"x": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.searchQuery) {
				app.update(st => ({ ...st, searchQuery: "", page: 1, selectedIndex: 0 }));
			}
		},
		"r": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			loadFeatures();
		},
		"c": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "sheet" && s.sheetOpen && s.selectedItem) {
				clipboardy.writeSync(s.selectedItem.id);
				app.update(st => ({ ...st, copiedFeedback: true }));
				setTimeout(() => app.update(st => ({ ...st, copiedFeedback: false })), 2000);
			}
		},
		"o": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "sheet" && s.sheetOpen && s.selectedItem) {
				open(`${AUTUMN_DASHBOARD_URL}/sandbox/features`);
			}
		},
		"n": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				const pg = getPageData(s);
				if (pg.hasMore) app.update(st => ({ ...st, page: st.page + 1, selectedIndex: 0 }));
			}
		},
		"p": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				const pg = getPageData(s);
				if (pg.hasPrev) app.update(st => ({ ...st, page: st.page - 1, selectedIndex: 0 }));
			}
		},
		"left": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				const pg = getPageData(s);
				if (pg.hasPrev) app.update(st => ({ ...st, page: st.page - 1, selectedIndex: 0 }));
			}
		},
		"right": (ctx) => {
			const s = ctx.state;
			if (s.searchOpen) return;
			if (s.focusTarget === "table") {
				const pg = getPageData(s);
				if (pg.hasMore) app.update(st => ({ ...st, page: st.page + 1, selectedIndex: 0 }));
			}
		},
	});

	// ── Start ──────────────────────────────────────────────────

	await app.start();
	await loadFeatures();
}
