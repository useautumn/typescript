	...
	useEffect(() => {
		(async () => {
			...
			for (let productId of productsToDelete) {
				...
				setDeleted([...deleted, `Product [${productId}] deleted successfully!`]);
			}
			...
			for (let feature of features) {
				...
				setCreated([...created, `Feature [${feature.id}] created successfully!`]);
			}
			for (let product of products) {
				setCreationState(`Pushing product [${product.id}]`);
				await upsertProduct(product);
				setCreationState(`Pushed product [${product.id}]`);
				setCreated([...created, `Product [${product.id}] created successfully!`]);
			}
			...
			for (let featureId of featuresToDelete) {
				...
				setDeleted([...deleted, `Feature [${featureId}] deleted successfully!`]);
			}
		})();
	}, []);
