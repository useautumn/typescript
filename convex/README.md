...
Errors generated by this package are `ConvexError` values whose data contains
only:
...
Set `timeoutMs` explicitly. Without it, only `check` and `track` have the SDK's
five-second timeout; every other operation waits indefinitely. If a call
outlives Convex's action execution limit, the platform reports its failure
instead of an `AUTUMN_INDETERMINATE` error.
