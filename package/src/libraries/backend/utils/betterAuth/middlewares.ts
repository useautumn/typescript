...
import { AutumnOptions } from "./types";

export const organizationMiddleware = createAuthMiddleware(async (ctx) => {
	...
	if (orgId && session) {
		...
		if (ownerMembers.length > 0) {
			const ownerUser = await ctx.context.adapter.findOne({
				model: "user",
				where: [
					{ field: "id", value: (ownerMembers[0] as Member).userId as string },
				],
			});
